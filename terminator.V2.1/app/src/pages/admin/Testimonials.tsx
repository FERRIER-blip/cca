...
export default function AdminTestimonials() {
  ...
  // Mutation pour l'approbation
  const approveMutation = useMutation({
    mutationFn: (t: Testimonial) => testimonialsAPI.update(t.id, { is_approved: !t.is_approved }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-testimonials'] });
      toast.success('Statut de visibilité mis à jour');
    },
  });

  // Mutation pour la mise en avant (Vedette)
  const featureMutation = useMutation({
    mutationFn: (t: Testimonial) => testimonialsAPI.update(t.id, { is_featured: !t.is_featured }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-testimonials'] });
                ...
                <div className="mt-6 flex flex-wrap justify-between items-center gap-4 border-t pt-4">
                  <div className="flex gap-2">
                    ...
                    <Button 
                      size="sm" 
                      variant="outline"
                      onClick={() => featureMutation.mutate(t)}
