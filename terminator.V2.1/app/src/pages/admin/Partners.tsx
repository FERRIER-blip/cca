...
export default function AdminPartners() {
  ...
  return (
    <div className="space-y-6">
          ...
          <motion.div key={partner.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {partner.website && <a href={partner.website} target="_blank" rel="noopener noreferrer" className="text-xs text-[#ff6f00] hover:underline">Visiter le site</a>}
              </div>
            </div>
            <div className="flex gap-1">
              <Button size="sm" variant="outline"><Edit2 className="w-3 h-3" /></Button>
              <Button size="sm" variant="outline" className="text-red-500" onClick={() => deleteMutation.mutate(partner.id)}>
      ...
      {!isLoading && !partners?.length && <p className="text-center text-gray-500 py-8">Aucun partenaire trouvé.</p>}
    </div>
  );
}
