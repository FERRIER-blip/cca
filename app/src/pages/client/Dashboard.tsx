...
import { 
  GraduationCap, Loader2, BookOpen, Clock, 
...
export default function ClientDashboard() {
  ...
  // Récupération des inscriptions avec les vraies données de progression
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] py-8">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <p className="text-gray-500">
              Ravi de vous revoir, <span className="font-semibold text-[#ff6f00]">{user?.first_name}</span> 👋
            </p>
          </motion.div>
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border ${statusColors[enrollment.status]}`}>
                            {enrollment.status === 'active' ? 'En cours' : enrollment.status === 'completed' ? 'Terminé' : enrollment.status}
                          </span>
                        ...
                        <div className="mt-3 w-full md:max-w-xs">
                          ...
                          <div className="h-1.5 w-full bg-gray-200 rounded-full overflow-hidden">
                            <motion.div 
                              ...
                              className={`h-full ${enrollment.status === 'completed' ? 'bg-green-500' : 'bg-[#ff6f00]'}`}
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 md:mt-0 flex gap-2">
                        {enrollment.status === 'completed' ? (
                          ...
                          <Button size="sm" asChild className="rounded-lg bg-[#1a237e] hover:bg-[#0d1245] shadow-md transition-transform active:scale-95">
                            <Link to={`/formations/${enrollment.training.slug}/learn`}>
        ...
        {/* Support Section */}
        <div className="mt-8 flex flex-col md:flex-row items-center justify-between p-6 bg-gradient-to-r from-[#1a237e] to-[#283593] rounded-3xl text-white shadow-xl">
          <div className="mb-4 md:mb-0 text-center md:text-left">
            <h4 className="font-bold text-lg">Besoin d'assistance technique ?</h4>
            <p className="text-blue-100 text-sm opacity-90">Nos conseillers pédagogiques sont disponibles pour vous aider.</p>
          </div>
          <Button asChild className="bg-white text-[#1a237e] hover:bg-blue-50 rounded-xl px-6 font-bold shadow-lg transition-transform active:scale-95">
